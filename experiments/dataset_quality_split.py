...
parser.add_argument('--quality-key', '-q', default='fitnessPP', help='Adjust this to match the'
...
if args.keep_data_source:
    ...
    for data in data_origins.values():
        sorted_set = sorted(data, key=lambda x: x[args.quality_key])
        low_quality_dataset += sorted_set[:(len(sorted_set) // 2)]
        high_quality_dataset += sorted_set[(len(sorted_set) // 2):]
else:
    sorted_train_set = sorted(entire_dataset, key=lambda x: x[args.quality_key])
